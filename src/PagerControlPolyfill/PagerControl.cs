	...
	public partial class PagerControl : Control
	{
		...
		protected override void OnApplyTemplate()
		{
			...
			if (GetTemplateChild(c_firstPageButtonName) is Button firstPageButtonTemplate)
			{
				...
				firstPageButton.Click += FirstPageButton_Click;
				AutomationProperties.SetName(firstPageButtonTemplate, ResourceAccessor.GetLocalizedStringResource(SR_PagerControlFirstPageButtonTextName));
			}
			if (GetTemplateChild(c_previousPageButtonName) is Button previousPageButtonTemplate)
			{
				...
				previousPageButton.Click += PreviousPageButton_Click;
				AutomationProperties.SetName(previousPageButtonTemplate, ResourceAccessor.GetLocalizedStringResource(SR_PagerControlPreviousPageButtonTextName));
			...
			if (GetTemplateChild(c_nextPageButtonName) is Button nextPageButtonTemplate)
			{
				...
				nextPageButton.Click += NextPageButton_Click;
				AutomationProperties.SetName(nextPageButtonTemplate, ResourceAccessor.GetLocalizedStringResource(SR_PagerControlNextPageButtonTextName));
			}
			if (GetTemplateChild(c_lastPageButtonName) is Button lastPageButtonTemplate)
			{
				lastPageButton = lastPageButtonTemplate;
				lastPageButton.Click += LastPageButton_Click;
				AutomationProperties.SetName(lastPageButtonTemplate, ResourceAccessor.GetLocalizedStringResource(SR_PagerControlLastPageButtonTextName));
			}
			if (GetTemplateChild(c_comboBoxName) is ComboBox comboBoxTemplate)
			{
				...
				AutomationProperties.SetName(comboBox, ResourceAccessor.GetLocalizedStringResource(SR_PagerControlPageTextName));
			}
			if (GetTemplateChild(c_numberBoxName) is NumberBox numberBoxTemplate)
			{
				...
				AutomationProperties.SetName(numberBox, ResourceAccessor.GetLocalizedStringResource(SR_PagerControlPageTextName));
			}
		...
		private void AppendButtonToNumberPanelList(int pageNumber, int numberOfPages)
		{
			...
			if(ResourceAccessor.ResourceLookup(c_numberPanelButtonStyleName) is Style style)
			{
				button.Style = style;
			}
